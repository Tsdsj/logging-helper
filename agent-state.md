...
- Completed `built-in-framework-sample-logs`: built-in sample logs now live in `js/sample-logs.mjs`, the upload area has a sample selector with an all-samples option, and coverage verifies Spring Boot, FastAPI, Node/Pino, Go/Zap, ASP.NET Core, and Android logcat samples parse with varied levels, failures, and timestamps.
- Completed `expanded-diagnostics-knowledge-base`: built-in diagnostics now cover many more JVM, Python, Node, Go, .NET, PHP, Ruby, database, messaging, container, Kubernetes, TLS, and DNS errors, with regression coverage for representative matches.
