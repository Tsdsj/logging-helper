...
- **自定义解析规则（高级）**：用正则覆盖默认的级别 / 时间戳 / 错误类型识别，规则保存在本地浏览器（localStorage），刷新后仍保留，修改后自动重新分析。详见下文「自定义规则」。
...
在上传区的「自定义解析规则（高级）」面板中，可用正则覆盖默认识别。规则保存在浏览器 localStorage，刷新后仍保留，修改后自动重新分析；非法正则会就地提示且不影响其余分析。

- **级别规则**（可多条，按从上到下顺序，优先于默认）：行匹配该正则时，判为所选级别。例如把含 `DENIED` 的行判为 `ERROR`：正则填 `\bDENIED\b`，级别选 `ERROR`。
- **时间戳规则**（优先于默认时间戳识别）：用**命名捕获组** `y`/`mo`/`d`/`h` 提取年（可选）、月、日、小时。例如 `08/21 14:` 这样的格式：`(?<mo>\d{2})\/(?<d>\d{2}) (?<h>\d{2}):`。需至少能取到 `h` 以及 `mo`+`d`（或完整 `y`+`mo`+`d`）才会生效。
- **错误类型规则**（优先于默认错误类型提取）：用捕获组 1（或命名组 `type`）作为错误类型。例如按错误码归类：`code=(\d+)`。
